...
from src.lib.tracking_utils.utils import xyxy2xywh, xywh2xyxy
...
class LoadImagesAndLabels:  # for training
    ...
    def get_data(self, img_path, label_path):
        ...
        if os.path.isfile(label_path):
            ...
            if labels0.shape[-1] == 6:
                labels0 = labels0.reshape(-1, 6)
            elif labels0.shape[-1] == 7:
                # for vis attr
                labels0 = labels0.reshape(-1, 7)
                mask = labels0[:, -1] >= 0.5
                print('> 0.5')
        ...
        if nL > 0:
            ...
            # labels[:, 2:6] = labels[:, 2:6].copy()  # / height
            labels[:, 2] /= width
            labels[:, 3] /= height
            labels[:, 4] /= width
            labels[:, 5] /= height
        ...
        # TODO: to debug
        labels[:, 2:6] = xywh2xyxy(labels[:, 2:6].copy())
...
def collate_fn(batch) -> (torch.Tensor, list):
    img_tensors, labels = [], []
    for i, sample in enumerate(batch):
        img_tensors.append(sample['input'])
        labels.append(torch.FloatTensor(sample['label']))

    img_tensors = torch.stack(img_tensors, dim=0)
    labels = torch.stack(labels, dim=0)

    return {'input': img_tensors, 'label': labels}


class AdaJointDataset(LoadImagesAndLabels):  # for training
    ...
    def __getitem__(self, files_index):
        ...
        max_labels = -np.ones((self.max_objs, 6), dtype=np.float32)
        max_labels[:len(labels), :] = labels
        # print(max_labels)

        ret = {'input': imgs, 'label': max_labels}
