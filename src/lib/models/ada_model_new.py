...
class AdaptiveNet(nn.Module):
    ...
    def __init__(self, device=None, backbone_name='dla', layers=34, num_class=80, stride=4, head_conv=256, gn=False,
                 leaky_relu=False,
        ...
        assert backbone_name in ('resnet', 'resnet_fpn', 'dla')
        ...
        if self.gn:
            cls_branch.append(nn.GroupNorm(32, self.head_conv))
            ltrb_branch.append(nn.GroupNorm(32, self.head_conv))
            id_branch.insert(1, nn.GroupNorm(32, self.head_conv))

        self.cls_conv = nn.Sequential(*cls_branch)
        self.ltrb_conv = nn.Sequential(*ltrb_branch)
        self.id_conv = nn.Sequential(*id_branch)

        self.cls_head = nn.Conv2d(self.head_conv, self.num_class, kernel_size=1, padding=0)
        self.ltrb_head = nn.Conv2d(self.head_conv, 4, kernel_size=1, padding=0)
        self.id_head = nn.Conv2d(self.head_conv, self.emb_dim, kernel_size=1, padding=0)
    ...
    def _reset_parameters(self, *heads):
        ...
        nn.init.constant_(self.cls_head.bias, self.bias_value)
    ...
    def forward(self, imgs):
        ...
        B, _, H, W = hm4.shape
        cls_conv = self.cls_conv(hm4)
        ltrb_conv = self.ltrb_conv(hm4)
        id_conv = self.id_conv(hm4)
        # heads
        cls_pred_logits = self.cls_head(cls_conv)  # [B, num_cls, H, W]
        ltrb_pred_logits = self.ltrb_head(ltrb_conv)
        id_pred_logits = self.id_head(id_conv)
